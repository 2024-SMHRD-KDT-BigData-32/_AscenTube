...
public class YoutubeAnalyticsService {
	...
	private static final List<String> KNOWN_METRICS = Arrays.asList(
            "views", "estimatedMinutesWatched", "subscribersGained", "averageViewDuration",
            "likes", "comments", "adImpressions", "viewerPercentage"
    );
	...
	/*
	 * public QueryResponse getChannelBasicAnalytics(String startDate, String
	 * endDate, String channelId) throws IOException, GeneralSecurityException { //
	 * 저장된 Credential 로드 Credential credential = oAuthService.getCredential();
	 * if(credential == null || credential.getAccessToken() == null) { throw new
	 * GeneralSecurityException("OAuth 인증이 필요합니다. /oauth/google/authorize 로 인증을 시작하세요."
	 * ); }
	 * 
	 * // 인증된 YouTubeAnalytics 서비스 객체 생성 YouTubeAnalytics analytics =
	 * oAuthService.getYouTubeAnalyticsService(credential);
	 * 
	 * // Analytics API 쿼리 빌드 YouTubeAnalytics.Reports.Query request =
	 * analytics.reports().query();
	 * 
	 * request.setIds("channel==" + (channelId != null && !channelId.isEmpty() ?
	 * channelId : "mine")); // IDs 설정 request.setStartDate(startDate); // 시작 날짜 설정
	 * request.setEndDate(endDate); // 종료 날짜 설정 request.setMetrics(
	 * "views,estimatedMinutesWatched,subscribersGained,averageViewDuration"); //
	 * 측정항목 설정 request.setDimensions("day"); request.setSort("day");
	 * 
     ...
     */
	 public Map<String, Map<String, Object>> parseAnalyticsResult(QueryResponse queryResponse) {
	        ...
	        List<String> columnNames = queryResponse.getColumnHeaders().stream()
	        ...
	        if (dimensionIndices.isEmpty()) {
	            logger.error("No dimension columns found in headers: {}. A report must have at least one dimension for parsing.", columnNames);
	            return Collections.emptyMap();
	        }
	        if (metricIndices.isEmpty()) {
	            logger.error("No metric columns found in headers: {}. A report must have at least one metric for parsing.", columnNames);
	            return Collections.emptyMap();
	        }
		...
		/*
		 * public QueryResponse getViewsByCountry(String startDate, String endDate,
		 * String channelId) throws IOException, GeneralSecurityException { Credential
		 * credential = oAuthService.getCredential(); if(credential == null ||
		 * credential.getAccessToken() == null) { throw new
		 * GeneralSecurityException("OAuth 인증이 필요합니다."); }
		 * 
		 ...
		 * request.setIds("channel==" + (channelId != null && !channelId.isEmpty() ?
		 * channelId : "mine")); request.setStartDate(startDate);
		 * request.setEndDate(endDate); request.setMetrics("views");
		 * request.setDimensions("country"); request.setSort("-views");
		 * 
		 * return request.execute(); }
		 */
     ...
     */
	/*
	 * public QueryResponse getTrafficSourceType(String startDate, String endDate,
	 * String channelId) throws IOException, GeneralSecurityException { Credential
	 * credential = oAuthService.getCredential(); if (credential == null ||
	 * credential.getAccessToken() == null) { throw new
	 * GeneralSecurityException("OAuth 인증이 필요합니다."); } YouTubeAnalytics analytics =
	 * oAuthService.getYouTubeAnalyticsService(credential);
	 * 
	 * YouTubeAnalytics.Reports.Query request = analytics.reports().query();
	 * request.setIds("channel==" + (channelId != null && !channelId.isEmpty() ?
	 ...
	 * request.setFilters("continent==019"); request.setSort("-views"); //
