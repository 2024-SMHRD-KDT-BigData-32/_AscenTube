...
public class YoutubeDataApiService {
	...
	// 기존 필드들은 유지
	private final AscenTubeApplication ascenTubeApplication; // 이 필드의 용도는 현재 코드에서 명확하지 않으나 일단 유지
	private final YouTube youTubeService; // 필드명을 youtubeService에서 youTubeService로 변경 (일관성 및 명확성)
	...
	public YoutubeDataApiService(YouTube youTube, GoogleApiConfig googleApiConfig,
		...
		this.youTubeService = youTube; // 주입받은 YouTube 객체 사용
		this.googleApiConfig = googleApiConfig; // API 키 접근을 위해 googleApiConfig 주입
